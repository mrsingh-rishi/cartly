...
describe("Cartly API", () => {
  ...
  it("blocks coupon generation before three orders", async () => expect((await request(app).post("/api/admin/discount-codes")).body.error.code).toBe("COUPON_NOT_ELIGIBLE"));
  it("generates a coupon after three orders", async () => { await placeOrders(3); expect((await request(app).post("/api/admin/discount-codes")).body.coupon.discountPercent).toBe(10); });
  it("blocks duplicate generation for a milestone", async () => { await placeOrders(3); await request(app).post("/api/admin/discount-codes"); expect((await request(app).post("/api/admin/discount-codes")).body.error.code).toBe("COUPON_ALREADY_GENERATED_FOR_MILESTONE"); });
  it("applies a generated coupon", async () => { await placeOrders(3); const code = (await request(app).post("/api/admin/discount-codes")).body.coupon.code; await add(); expect((await checkout(code)).body.order.discountCents).toBe(500); });
  it("prevents coupon reuse", async () => { await placeOrders(3); const code = (await request(app).post("/api/admin/discount-codes")).body.coupon.code; await add(); await checkout(code); await add(); expect((await checkout(code)).body.error.code).toBe("COUPON_ALREADY_USED"); });
  it("rejects invalid coupons", async () => { await add(); expect((await checkout("NOPE")).body.error.code).toBe("INVALID_COUPON"); });
  it("reports order and item stats", async () => { await add("1", 2); await checkout(); const stats = (await request(app).get("/api/admin/stats")).body.stats; expect(stats.totalOrders).toBe(1); expect(stats.totalItemsPurchased).toBe(2); });
  it("reports actual revenue", async () => { await add("1"); await checkout(); expect((await request(app).get("/api/admin/stats")).body.stats.totalRevenueCents).toBe(5798); });
  it("reports generated codes and discounts given", async () => { await placeOrders(3); const code = (await request(app).post("/api/admin/discount-codes")).body.coupon.code; await add(); await checkout(code); const stats = (await request(app).get("/api/admin/stats")).body.stats; expect(stats.discountCodesGenerated).toBe(1); expect(stats.totalDiscountsGivenCents).toBe(500); });
});
